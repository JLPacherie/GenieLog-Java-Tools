...
public abstract class AConfig<C extends AChecker> extends AttributeWrapper {
  ...
  protected transient List<String> loggedPackages = new ArrayList<>();
  ...
  protected List<C> _disabledCheckers = new ArrayList<>();
  ...
  protected AConfig() {
    ...
    addLoggedPackage(AConfig.class.getPackage().getName());
    addLoggedPackage(this.getClass().getPackage().getName());

  }

  public void addLoggedPackage(String name) {
    if (!loggedPackages.contains(name)) {
      loggedPackages.add(name);
    }
  }
  ...
  protected C makeChecker(JsonNode jsonChecker) {
    ...
    int iPackage = 0;
    while ((iPackage < loggedPackages.size()) && (checkerClass == null)) {
      String fullClassname = loggedPackages.get(iPackage) + "." + className;
      try {
  ...
  public boolean cliLoad(String[] args) {
    ...
    if (result) {
      ...
      try {
        ...
        // ------------------------------------------------------------------------------------------------------------
        // Deactivating Loggers to Console
        // ------------------------------------------------------------------------------------------------------------

        if (line.hasOption("quiet") || line.hasOption("verbose") || line.hasOption("debug")) {

          LoggerContext ctx = (LoggerContext) LogManager.getContext(false);
          Configuration config = ctx.getConfiguration();

          boolean verbose = !line.hasOption("quiet") || line.hasOption("verbose");
          boolean debug = line.hasOption("debug");

          Map<String, LoggerConfig> allLoggers = config.getLoggers();
          allLoggers.entrySet()
              .stream()
              // Only modify the registered loggers
              .filter(entry -> loggedPackages.stream().anyMatch(name -> name.contains(entry.getKey())))
              .forEach(entry -> {
                String loggerId = entry.getKey();
                LoggerConfig loggerConfig = entry.getValue();

                // If all appenders are impacted then use :
                if (debug) {
                ...
                // Only modify the Appenders with Console output
                List<String> consoleAppenderNames = loggerConfig.getAppenders().keySet()
                    .stream()
                    .filter(appenderName -> appenderName.contains("Console"))
                    .collect(Collectors.toList());

                consoleAppenderNames.forEach(appenderName -> {
                  ConsoleAppender consoleAppender = (ConsoleAppender) loggerConfig.getAppenders().get(appenderName);
                  if (consoleAppender != null) {
                    ...
                    if (debug) {
                      ...
                      ThresholdFilter newFilter = ThresholdFilter.createFilter(Level.INFO,
                          verbose ? Result.ACCEPT : Result.DENY,
                          verbose ? Result.DENY : Result.ACCEPT);
                      consoleAppender.addFilter(newFilter);
                    }

                    _logger.debug("Deactivating Console appenders named {} on Logger {}", appenderName, loggerId);
                  }
                });
              });
        ...
        // ----------------------------------------------------------------
        // Overwriting some JSON parameters of the Configuration file
        ...
        // Disable Checkers - this disable the given checker
        {
          ...
          if (checkerNameList != null) {
            for (String checkerName : checkerNameList) {
              ...
              if (checker == null) {
                ...
                _logger.debug("Manually disabling checker: {}", checker);
              }
            }
          }
        }
        ...
        if (line.hasOption("list-checkers")) {
          ...
          for (C checker : _disabledCheckers) {
  ...
  public boolean init() {
    ...
    _logger.debug("Clearing list of {} available checkers.", _disabledCheckers.size());
    _disabledCheckers.clear();
  ...
  public boolean isValid() {
    ...
    // ----------------------------------------
    if (_enabledCheckers.size() + _disabledCheckers.size() == 0) {
      result = false;
  ...
  public boolean enableAllCheckers() {
    ...
    while (result && !_disabledCheckers.isEmpty()) {
      // Enabling a checker removes it from the available list.
      result = enableChecker(_disabledCheckers.get(0).getName()) != null;
    }
  ...
  public C enableChecker(String name) {
    ...
    C checker = getDisabledChecker(name);
    if ((checker != null) && (checker.isValid())) {
      _enabledCheckers.add(checker);
      _disabledCheckers.remove(checker);
  ...
  public C getChecker(String name) {
    C result = getDisabledChecker(name);
    return result != null ? result : getEnabledChecker(name);
  }

  public Stream<C> getAllCheckers() {
    return Stream.concat(getEnabledCheckers(), getDisabledCheckers());
  }
   ...
   */
  public C getDisabledChecker(String name) {
    if (!isValidCheckerName(name)) {
      throw new IllegalArgumentException("Checker's name must be non null and not empty");
    }
    return _disabledCheckers.stream().filter(c -> name.equals(c.getName())).findFirst().orElse(null);
  }

  public Stream<C> getDisabledCheckers() {
    return _disabledCheckers.stream();
  }
  ...
  public Stream<C> getEnabledCheckers() {
